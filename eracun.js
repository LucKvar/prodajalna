...
streznik.get('/', function(zahteva, odgovor) {
  if(zahteva.session.uspelaPrijava) {
...
var vrniPodrobnostiOStranki = function(strankaId, callback) {
  pb.all("SELECT Customer.* FROM Customer, Invoice \
...
streznik.post('/izpisiRacunBaza', function(zahteva, odgovor) {
  var form = new formidable.IncomingForm();
    ...
    form.parse(zahteva, function (napaka1, polja, datoteke) {
      var stStranke = polja.seznamRacunov;
        strankaIzRacuna(stStranke, function (podatkiOStranki) {
          pesmiIzRacuna(stStranke, function (podatkiOPesmih) {
            odgovor.setHeader('content-type', 'text/xml');
            odgovor.render('eslog', {vizualiziraj:true, stranka:podatkiOStranki, postavkeRacuna:podatkiOPesmih});
          });
        });
    });
})
...
streznik.get('/izpisiRacun/:oblika', function(zahteva, odgovor) {
  pesmiIzKosarice(zahteva, function(pesmi) {
    if (!pesmi) {
      ...
      vrniPodrobnostiOStranki(zahteva.session.idTrenutneStranke, function(stranka) {
        odgovor.setHeader('content-type', 'text/xml');
...
streznik.post('/prijava', function(zahteva, odgovor) {
  ...
  form.parse(zahteva, function (napaka1, polja, datoteke) {
    ...
    if(napaka2) {
      odgovorNaRegistracijo = "Prišlo je do napake pri registraciji nove stranke. Prosim preverite vnešene podatke in poskusite znova.";
    } else {
      odgovorNaRegistracijo = "Stranka je bila uspešno registrirana."
    }
    
    vrniStranke(function(napaka1, stranke) {
        vrniRacune(function(napaka2, racuni) {
            odgovor.render('prijava', {sporocilo: odgovorNaRegistracijo, seznamStrank: stranke, seznamRacunov: racuni});
        })
    });
    //odgovor.end();
  });
...
streznik.post('/stranka', function(zahteva, odgovor) {
  ...
  form.parse(zahteva, function (napaka1, polja, datoteke) {
    zahteva.session.uspelaPrijava = polja.seznamStrank;
    zahteva.session.idTrenutneStranke = polja.seznamStrank;
    odgovor.redirect('/')
  });
})
...
streznik.post('/odjava', function(zahteva, odgovor) {
  zahteva.session.uspelaPrijava = null;
  zahteva.session.idTrenutneStranke = null;
    odgovor.redirect('/prijava') 
})
